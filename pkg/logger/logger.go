...
func init() {
	...
	err := env.Parse(&cfg)
	if err != nil {
		log.Fatal().Msg(fmt.Sprintf("Failed to parse server environment: %v", err))
	}

	// translate log level
	v, ok := levelMap[strings.ToLower(cfg.LogLevel)]
	if !ok {
		log.Fatal().Msg(fmt.Sprintf("Unknown log level: %s", cfg.LogLevel))
	}
	...
	case "json":
		...
		// use default timestamp format (RFC3339, subset of iso8601) and UTC for json
		logOutput = os.Stdout
